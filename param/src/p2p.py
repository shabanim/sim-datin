...
from .network import P2P, Network
from .collective import AllReduce, AllGather, Gather, ReduceScatter, Scatter, Reduce, Broadcast, Collective, All2All
...
def local_metrics(network, reduce_gather_mdfi, reduce_gather_su, write_su):
    ...
    local_write_scale_up = write_su / \
                           min(network["dss_or_sms"] * network["achievable_write_L3_BW_with_congestion"],
                               network["max_achievable_read_memBW"])
...
@multimethod(P2P, AllReduce)
def comms(p2p, allreduce, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        if p2p["data_parallel"]:
            p2p["model_split"] = 1
        links = p2p["links"] #p2p["links"]/(math.ceil(p2p["num_PVC_per_host"]/ sockets))
    else:
        if p2p["num_PVC_per_host"] != 1:
            links_per_socket = math.floor(p2p["links"] / (p2p["num_PVC_per_host"] - 1))
            links = links_per_socket * (sockets - 1)
        else:
            links = 0
    #print(links)
    nsteps_scaleup = 1
    su_lat_us = su_latency_us(p2p, sockets)
    su_total_latency_us = su_total_latency(p2p, su_lat_us)

    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(p2p, message_size, tiles_per_socket, sockets)
    total_message_volume_bytes = 2 * (sockets - 1) * msg_size_per_scale_up_per_tile / sockets
    total_latency_for_scale_up = 2 * nsteps_scaleup * su_total_latency_us + p2p["sw_lat_us"]

    if p2p["use_full_message_size_in_HBM_latency_calculation"] == 1:
    ...
    remote_write_scale_up = 2 * (sockets - 1) * min(msg_size_per_scale_up_per_tile,
                                                    p2p["latency_threshold_message_size_for_scale_up"]) * \
                            1000000 / (sockets * Constants.GIGACONV)
    total_scale_up_BW = links * p2p["su_BW_per_link_ACHIEVABLE"]

    Effective_remote_write_BW_over_MDFI = min(p2p["max_achievable_write_memBW"],
                                              p2p["mdfi_BW_per_tile_ACHIEVABLE"])
    ...
    total_latency_for_MDFI = 2 * nsteps_MDFI * p2p["total_mdfi_latency"]

    if p2p["use_chunk_size_in_MDFI_latency_calculation"]:
    ...
    remote_write_MDFI = 2 * nsteps_MDFI * min(msg_size_per_MDFI_per_tile,
                                              p2p["latency_threshold_message_size_for_MDFI"]) * \
    ...
    try:
        final_remote_write_mdfi = remote_write_MDFI / \
                                  min(p2p["dss_or_sms"] * p2p["achievable_write_L3_BW_with_congestion"],
                                      p2p["max_achievable_read_memBW"], Effective_remote_write_BW_over_MDFI)
    ...
    if sockets != 1:
        final_remote_write_scale_up = remote_write_scale_up / min(
            (p2p["dss_or_sms"] * p2p["achievable_write_L3_BW_with_congestion"]),
            p2p["max_achievable_write_memBW"],
            min(p2p["dss_or_sms"], links) * p2p["su_BW_per_link_ACHIEVABLE"])
    ...
    try:
        final_mdfi_achieved_BW = 2 * min(msg_size_per_MDFI_per_tile, p2p["latency_threshold_message_size_for_MDFI"]) \
    ...
    try:
        ...
        final_scale_up_achieved_BW = 0

    stats = CommsStats()
    stats.mdfi_bw = (p2p["total_mdfi_BW"] * Constants.GIGACONV)
...
@multimethod(P2P, AllGather)
def comms(p2p, allgather, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        if p2p["data_parallel"]:
            p2p["model_split"] = 1
        links = p2p["links"] #p2p["links"]/(math.ceil(p2p["num_PVC_per_host"]/ sockets))
    ...
    total_latency_for_scale_up = nsteps_scaleup * su_total_latency_us + p2p["sw_lat_us"]
    ...
    msg_size_per_MDFI_per_tile = 0 if not no_of_concurrent_messages_over_mdfi else message_size / no_of_concurrent_messages_over_mdfi
    nsteps_MDFI = tiles_per_socket - 1
    total_latency_for_MDFI = nsteps_MDFI * p2p["total_mdfi_latency"]
    ...
    try:
        final_mdfi_achieved_BW = min(msg_size_per_MDFI_per_tile, p2p["latency_threshold_message_size_for_MDFI"]) \
                                 * nsteps_MDFI / (tiles_per_socket * final_mdfi_time_us / 1000000)
    except ZeroDivisionError:
        final_mdfi_achieved_BW = 0
...
@multimethod(P2P, Gather)
def comms(p2p, gather, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        links = p2p["links"]
    else:
        if sockets == 4:
...
@multimethod(P2P, ReduceScatter)
def comms(p2p, reducescatter, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        links = p2p["links"]
    else:
        if sockets == 4:
...
@multimethod(P2P, Scatter)
def comms(p2p, scatter, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        links = p2p["links"]
    else:
        if sockets == 4:
...
@multimethod(P2P, Reduce)
def comms(p2p, reduce, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        if p2p["data_parallel"]:
            p2p["model_split"] = 1
        links = p2p["links"] #p2p["links"]/(math.ceil(p2p["num_PVC_per_host"]/ sockets))
    ...
    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(p2p, message_size, 1, sockets)
...
@multimethod(P2P, Broadcast)
def comms(p2p, broadcast, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        links = p2p["links"]
    else:
        if sockets == 4:
...
@multimethod(P2P, All2All)
def comms(p2p, all2all, message_size, tiles_per_socket, sockets):
    if p2p["su_option"] == 'SWITCH':
        links = p2p["links"]
    else:
        if sockets == 4:
    ...
    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(p2p, message_size, tiles_per_socket, sockets)
    total_message_volume_bytes = (sockets - 1) * message_size / sockets
    total_latency_for_scale_up = 1 * nsteps_scaleup * su_total_latency_us + p2p["sw_lat_us"]
    ...
    remote_write_scale_up = (sockets - 1) * message_size * \
                            1000000 / (sockets * Constants.GIGACONV)
    total_scale_up_BW = links * p2p["su_BW_per_link_ACHIEVABLE"]
    ...
    msg_size_per_MDFI_per_tile = 0 if not no_of_concurrent_messages_over_mdfi else message_size / no_of_concurrent_messages_over_mdfi
    nsteps_MDFI = tiles_per_socket - 1
    total_latency_for_MDFI = 1 * nsteps_MDFI * p2p["total_mdfi_latency"]
    ...
    remote_write_MDFI = (tiles_per_socket - 1) * msg_size_per_MDFI_per_tile * \
                        1000000 / (tiles_per_socket * Constants.GIGACONV)
