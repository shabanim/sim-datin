import math
from .collective import AllReduce, AllGather, Gather, ReduceScatter, Scatter, Reduce, Broadcast
...
@multimethod(Hypercube, AllReduce)
def comms(hypercube, allreduce, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
    nsteps_scaleup = math.log(sockets, 2)
    su_lat_us = su_latency_us(hypercube, sockets)
    su_total_latency_us = su_total_latency(hypercube, su_lat_us)

    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(hypercube, message_size, tiles_per_socket, sockets)
    total_message_volume_bytes = 2 * (sockets - 1) * msg_size_per_scale_up_per_tile / sockets
    total_latency_for_scale_up = 2 * nsteps_scaleup * su_total_latency_us + hypercube["sw_lat_us"]

    if hypercube["use_full_message_size_in_HBM_latency_calculation"] == 1:
    ...
    remote_write_scale_up = 2 * (sockets - 1) * min(msg_size_per_scale_up_per_tile,
                                                    hypercube["latency_threshold_message_size_for_scale_up"]) * \
                            1000000 / (sockets * Constants.GIGACONV)
    total_scale_up_BW = links * hypercube["su_BW_per_link_ACHIEVABLE"]

    Effective_remote_write_BW_over_MDFI = min(hypercube["max_achievable_write_memBW"],
                                              hypercube["mdfi_BW_per_tile_ACHIEVABLE"])
    ...
    total_latency_for_MDFI = 2 * nsteps_MDFI * hypercube["total_mdfi_latency"]

    if hypercube["use_chunk_size_in_MDFI_latency_calculation"]:
    ...
    remote_write_MDFI = 2 * nsteps_MDFI * min(msg_size_per_MDFI_per_tile,
                                              hypercube["latency_threshold_message_size_for_MDFI"]) * \
    ...
    try:
        final_remote_write_mdfi = remote_write_MDFI / \
                                  min(hypercube["dss_or_sms"] * hypercube["achievable_read_L3_BW_with_congestion"],
                                      hypercube["max_achievable_read_memBW"], Effective_remote_write_BW_over_MDFI)
    except ZeroDivisionError:
        final_remote_write_mdfi = 0

    final_remote_write_scale_up = remote_write_scale_up / min(
        (hypercube["dss_or_sms"] * hypercube["achievable_write_L3_BW_with_congestion"]),
        hypercube["max_achievable_write_memBW"],
        min(hypercube["dss_or_sms"], links) * hypercube["su_BW_per_link_ACHIEVABLE"])
    ...
    stats = CommsStats()
    stats.mdfi_bw = (hypercube["total_mdfi_BW"] * Constants.GIGACONV)
...
@multimethod(Hypercube, AllGather)
def comms(hypercube, allgather, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
    ...
    total_latency_for_scale_up = nsteps_scaleup * su_total_latency_us + hypercube["sw_lat_us"]
    ...
    msg_size_per_MDFI_per_tile = 0 if not no_of_concurrent_messages_over_mdfi else message_size / no_of_concurrent_messages_over_mdfi
    nsteps_MDFI = tiles_per_socket - 1
    total_latency_for_MDFI = nsteps_MDFI * hypercube["total_mdfi_latency"]
    ...
    try:
        final_mdfi_achieved_BW = min(msg_size_per_MDFI_per_tile, hypercube["latency_threshold_message_size_for_MDFI"]) \
                                 * nsteps_MDFI / (tiles_per_socket * final_mdfi_time_us / 1000000)
    except ZeroDivisionError:
        final_mdfi_achieved_BW = 0
...
@multimethod(Hypercube, Gather)
def comms(hypercube, gather, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
...
@multimethod(Hypercube, ReduceScatter)
def comms(hypercube, reducescatter, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
...
@multimethod(Hypercube, Scatter)
def comms(hypercube, scatter, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
...
@multimethod(Hypercube, Reduce)
def comms(hypercube, reduce, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
    ...
    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(hypercube, message_size, 1, sockets)
...
@multimethod(Hypercube, Broadcast)
def comms(hypercube, broadcast, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = hypercube["links"]
