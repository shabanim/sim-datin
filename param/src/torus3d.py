...
from .network import Torus3d, Network
from .collective import AllReduce, AllGather, Gather, Reduce, Scatter, ReduceScatter, Broadcast
...
@multimethod(Torus3d, AllReduce)
def comms(torus_3d, allreduce, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
    nsteps_scaleup = (max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"], torus_3d["zdim_3d_torus"]) - 1) * 3
    su_lat_us = su_latency_us(torus_3d, sockets)
    su_total_latency_us = su_total_latency(torus_3d, su_lat_us)

    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(torus_3d, message_size, tiles_per_socket, sockets)
    total_message_volume_bytes = 2 * (max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"],
                                          torus_3d["zdim_3d_torus"]) - 1) * msg_size_per_scale_up_per_tile / sockets
    total_latency_for_scale_up = 2 * nsteps_scaleup * su_total_latency_us + torus_3d["sw_lat_us"]

    if torus_3d["use_full_message_size_in_HBM_latency_calculation"] == 1:
    ...
    remote_write_scale_up = 2 * (
                max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"], torus_3d["zdim_3d_torus"]) - 1) * min(
        msg_size_per_scale_up_per_tile,
        torus_3d["latency_threshold_message_size_for_scale_up"]) * \
                            1000000 / (max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"],
                                           torus_3d["zdim_3d_torus"]) * Constants.GIGACONV)
    total_scale_up_BW = links * torus_3d["su_BW_per_link_ACHIEVABLE"]

    Effective_remote_write_BW_over_MDFI = min(torus_3d["max_achievable_write_memBW"],
                                              torus_3d["mdfi_BW_per_tile_ACHIEVABLE"])
    ...
    total_latency_for_MDFI = 2 * nsteps_MDFI * torus_3d["total_mdfi_latency"]

    if torus_3d["use_chunk_size_in_MDFI_latency_calculation"]:
    ...
    remote_write_MDFI = 2 * nsteps_MDFI * min(msg_size_per_MDFI_per_tile,
                                              torus_3d["latency_threshold_message_size_for_MDFI"]) * \
    ...
    try:
        final_remote_write_mdfi = remote_write_MDFI / \
                                  min(torus_3d["dss_or_sms"] * torus_3d["achievable_read_L3_BW_with_congestion"],
                                      torus_3d["max_achievable_read_memBW"], Effective_remote_write_BW_over_MDFI)
    except ZeroDivisionError:
        final_remote_write_mdfi = 0

    final_remote_write_scale_up = remote_write_scale_up / min(
        (torus_3d["dss_or_sms"] * torus_3d["achievable_write_L3_BW_with_congestion"]),
        torus_3d["max_achievable_write_memBW"],
        min(torus_3d["dss_or_sms"], links) * torus_3d["su_BW_per_link_ACHIEVABLE"])
    ...
    try:
        final_mdfi_achieved_BW = 2 * min(msg_size_per_MDFI_per_tile,
                                         torus_3d["latency_threshold_message_size_for_MDFI"]) \
    ...
    stats = CommsStats()
    stats.mdfi_bw = (torus_3d["total_mdfi_BW"] * Constants.GIGACONV)
...
@multimethod(Torus3d, AllGather)
def comms(torus_3d, allgather, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
    ...
    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(torus_3d, message_size, tiles_per_socket, sockets)
    total_message_volume_bytes = (max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"],
                                      torus_3d["zdim_3d_torus"]) - 1) * msg_size_per_scale_up_per_tile / sockets
    total_latency_for_scale_up = nsteps_scaleup * su_total_latency_us + torus_3d["sw_lat_us"]
    ...
    remote_write_scale_up = (max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"],
                                 torus_3d["zdim_3d_torus"]) - 1) * min(msg_size_per_scale_up_per_tile,
                                                                       torus_3d[
                                                                           "latency_threshold_message_size_for_scale_up"]) * \
                            1000000 / (max(torus_3d["xdim_3d_torus"], torus_3d["ydim_3d_torus"],
                                           torus_3d["zdim_3d_torus"]) * Constants.GIGACONV)
    ...
    msg_size_per_MDFI_per_tile = 0 if not no_of_concurrent_messages_over_mdfi else message_size / no_of_concurrent_messages_over_mdfi
    nsteps_MDFI = tiles_per_socket - 1
    total_latency_for_MDFI = nsteps_MDFI * torus_3d["total_mdfi_latency"]
    ...
    try:
        final_mdfi_achieved_BW = min(msg_size_per_MDFI_per_tile, torus_3d["latency_threshold_message_size_for_MDFI"]) \
                                 * nsteps_MDFI / (tiles_per_socket * final_mdfi_time_us / 1000000)
    except ZeroDivisionError:
        final_mdfi_achieved_BW = 0
...
@multimethod(Torus3d, Gather)
def comms(torus_3d, gather, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
...
@multimethod(Torus3d, ReduceScatter)
def comms(torus_3d, reducescatter, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
...
@multimethod(Torus3d, Scatter)
def comms(torus_3d, scatter, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
...
@multimethod(Torus3d, Reduce)
def comms(torus_3d, reduce, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
    ...
    msg_size_per_scale_up_per_tile = su_msg_size_per_tile(torus_3d, message_size, 1, sockets)
...
@multimethod(Torus3d, Broadcast)
def comms(torus_3d, broadcast, message_size, tiles_per_socket, sockets):
    if sockets == 8:
        links = 6
    else:
        links = torus_3d["links"]
