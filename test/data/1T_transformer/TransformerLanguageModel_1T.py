...
class TransformerLanguageModel_1T(Network):
    def __init__(
            self,
            name,
            input_seq_len=2048,  # length of input sequence
            hidden_act='relu',  # Supports "relu" and "gelu"
            d_k=192,  # Dimensions of query and key inputs (section 3.2.1)
            d_v=192,  # Dimensions of value inputs (section 3.2.1)
            d_model=25600,  # Model dim (section 3.1)
            d_ff=98304,  # For full model - FFN inner layer dimension (section 3.3)
            num_enc_layers=1,  # Num of encoder layers
            num_heads=128,  # For full model - Num of attention heads (h, section 3.2.2)
            vocab_size=50280,  # For full model - Size of vocabulary in token embedding
            head_divide=128,  # Dividing factor for num_heads (model parallelism)
            d_ff_divide=128,  # Dividing factor for d_ff (model parallelism)
            vocab_divide=128,  # Dividing factor for vocab_size (model parallelism)
            verbose=False,
            userInput=None):
        '''
        ...
        The specific parameters values selected above follow the Megatron GPT3 1-Trillion
        Specification: "Scaling Language Model Training to a Trillion Parameters Using Megatron"
